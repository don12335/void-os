class VoidOS {
    ...
    async runBootSequence() {
        ...
        // Enter Sanctuary
        // Check Authentication before entering Sanctuary
    ...
    async checkAuthStatus() {
        try {
            const res = await fetch('/api/auth/check_init');
            const data = await res.json();

            if (data.initialized) {
                this.renderAuthScreen('login');
            } else {
                this.renderAuthScreen('register');
            }
        } catch (e) {
            console.error("Auth check failed:", e);
            // Fallback (e.g. offline mode) -> Just boot directly for now
            this.enterSanctuary();
        }
    }
    ...
    renderAuthScreen(type) {
        ...
        authScreen.style.cssText = `
            ...
            justify-content: center; align-items: center; color: var(--gold-mid);
            font-family: 'Crimson Text', serif;
        `;

        const title = type === 'login' ? 'IDENTIFY YOURSELF' : 'CREATE ADMIN ACCOUNT';
        const btnText = type === 'login' ? 'UNSEAL' : 'ESTABLISH';

        authScreen.innerHTML = `
            <div style="font-size: 2em; margin-bottom: 30px; letter-spacing: 2px;">${title}</div>
            <div style="width: 300px; display: flex; flex-direction: column; gap: 15px;">
                <input type="text" id="auth-user" placeholder="USERNAME" style="
                    background: rgba(20, 10, 5, 0.8); border: 1px solid var(--gold-dark);
                    padding: 10px; color: var(--gold-light); font-family: inherit;
                    text-align: center; font-size: 1.2em; outline: none;
                ">
                <input type="password" id="auth-pass" placeholder="PASSWORD" style="
                    background: rgba(20, 10, 5, 0.8); border: 1px solid var(--gold-dark);
                    padding: 10px; color: var(--gold-light); font-family: inherit;
                    text-align: center; font-size: 1.2em; outline: none;
                ">
                <button id="auth-btn" style="
                    background: var(--gold-dark); color: #000; border: none;
                    padding: 10px; font-size: 1.2em; cursor: pointer;
                    font-family: inherit; font-weight: bold; margin-top: 10px;
                    transition: all 0.3s ease;
                ">${btnText}</button>
                <div id="auth-msg" style="color: #ff4444; min-height: 20px; text-align: center;"></div>
            </div>
        `;
        ...
        const submit = async () => {
            ...
            try {
                ...
                if (data.success) {
                    authScreen.style.transition = 'opacity 1s';
                    authScreen.style.opacity = '0';
                    setTimeout(() => {
                        authScreen.remove();
                        // If registered, show login or just enter? 
                        // Let's just enter for valid login flow.
                        // If register success, maybe auto-login? 
                        // For simplicity: If register success -> Enter. 
                        this.enterSanctuary();
                    }, 1000);
                } else {
        ...
        btn.onclick = submit;
        passIn.onkeypress = (e) => { if (e.key === 'Enter') submit(); };
    ...
    /* --- APP LOGIC: TREASURY (FS) --- */
    async loadTreasuryPath(path = '') {
        try {
            ...
            const pathEl = document.getElementById('treasury-path');
            if (pathEl) pathEl.value = this.currentPath;
            ...
            data.files.forEach(file => {
                ...
                // Icon
                let iconClass = file.type === 'dir' ? 'fas fa-folder' : 'fas fa-file';
                if (file.name.endsWith('.png') || file.name.endsWith('.jpg')) iconClass = 'fas fa-image';

                item.innerHTML = `
                    <div style="font-size: 2em; color: ${file.type === 'dir' ? 'var(--gold-mid)' : '#ccc'};">
                        <i class="${iconClass}"></i>
                    </div>
                    <div style="font-size: 0.8em; margin-top:5px; word-break:break-word;">${file.name}</div>
                `;

                // Click Action
                item.onclick = () => {
                    if (file.type === 'dir') {
                        this.loadTreasuryPath(`${this.currentPath}/${file.name}`);
                    } else {
                        // Open File
                        fetch('/api/fs/open', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ path: `${this.currentPath}/${file.name}` })
                        });
                    }
                };
                ...
                // Drag Logic
                item.ondragstart = (e) => {
                    this.dragStart(e, 'file', { path: `${this.currentPath}/${file.name}`, name: file.name });
                };
    ...
    /* --- APP LOGIC: ORACLE (TERM) --- */
    async execOracle(cmd) {
